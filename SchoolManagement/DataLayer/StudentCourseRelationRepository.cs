...
public class StudentCourseRepository
{
    ...
    public void AddStudentCourses(StudentCourse studentCourse)
    {
        ...
        using var connection = new OdbcConnection(ConnectionString);
        var result = connection.Execute($"insert into StudentCourse (StudentId) values({studentCourse.StudentId}),(CourseId) values ({studentCourse.CourseId})");
    }

    public void UpdateStudentCourses(StudentCourse studentCourse)
    {
        using var connection = new OdbcConnection(ConnectionString);
        var result = connection.Execute($"insert into StudentCourse (StudentId) values({studentCourse.StudentId}),(CourseId) values ({studentCourse.CourseId}) where Id==({studentCourse.Id})");
    }
